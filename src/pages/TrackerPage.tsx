import { useState } from "react";
import { Button, Stack, Title, Divider, Container } from "@mantine/core";
import { v4 as uuidv4 } from "uuid";
import AddModal from "../components/Modal";

type Expense = {
  id: string;
  name: string;
  amount: number | string;
  category: string;
};

export default function ExpenseTracker() {
  const [opened, setOpened] = useState(false);
  const [expenses, setExpenses] = useState<Expense[]>([]);
  const categories = ["Food", "Transport", "Entertainment"];
  
  const modal = () => { opened ? setOpened(false) : setOpened(true) };
  const handleAddExpense = (name: string, amount: number | string, category: string) => {
    const newExpense: Expense = {
      id: uuidv4(),
      name,
      amount,
      category,
    };
    setExpenses((prevExpenses) => [...prevExpenses, newExpense]);
    setOpened(false);
  }

  const totalCost = expenses.map((expense) => total += expense.amount);

  return (
    <Container style={{ maxWidth: 600, margin: "auto", padding: 20 }}>
      <Title order={2} mb="md">
        Expense Tracker
      </Title>
      <Button onClick={modal} >Add Expense Item</Button>
      {/* Type additional AddExpenseModal here. */}
      {opened && (
        <AddModal opened={opened}/>
      )}
      
      <Divider my="md" />
      {/* Type additional total cost here. */}
      <Title order={4}>Total cost: {totalCost} Baht</Title>
      <Stack my="sm">{/* Type additional text here. */}</Stack>

      <Divider my="md" />
      {/* Type additional card here. */}
      <Stack>{/* Type additional expense card list here. */}</Stack>
    </Container>
  );
}
